...
class SparePartViewModel : ViewModel() {
            ...
            FirebaseFirestore.getInstance().collection("spare_parts")
                ...
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        ...
                        model.bikeType = document.data["type"].toString()
    ...
    fun setListSparePartByBikeType(bikeType: String) {
        listData.clear()

        try {
            FirebaseFirestore.getInstance().collection("spare_parts")
                .whereEqualTo("bikeType", bikeType)
                .get()
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        val model = SparePartModel()
    ...
    fun setListSparePartByQuery(query: String) {
        listData.clear()
